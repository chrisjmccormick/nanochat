"""chat_rl_fast_v2.py — V2: permanent KV pool, no gen/train memory handoff.

Identical to chat_rl_fast.py (v1) EXCEPT the VMM lend/reclaim protocol is gone:
the KV pool is sized to the MEASURED static-round regime (div5 peaked at 599 of
1592 blocks; default here 28 GB ~= 796 blocks, 1.33x that max) and stays
PERMANENTLY MAPPED. v1 needed the 56 GB pool and full train residency to
time-share one 80 GB card, so every round paid pool.reclaim() (with its
torch.cuda.empty_cache()) + pool.lend() — measured cost ~0.8 s vmm_s + ~0.9 s
first-pack tax (the flushed activation segments rebuilt inside pack 1; see
agent-ops nanochat/2026-07-23_0830am_diverse-sft-rl/TRAIN_THROUGHPUT.md).
V2 drops both; expected round wall ~10.5 -> ~8.8 s at unchanged rollout stats.
Capacity: pool 28 + train peak ~44.5 reserved + context ~3 = ~75.5 of 80 GB.
The pool-exhaustion guard stays fail-loud; margin over the observed worst round
is 1.33x (v1's was 2.7x) — a trip still means policy collapse, just sooner.

Comparison protocol: run the div5 config via run_rl_v2.sh; gates = (a) fits
through capture+warmup+gen (no OOM), (b) train line shows pack 1 ~= 0.37 s
(tax gone; TRAIN_TIMING_VERBOSE=1), (c) vmm 0.0 on the round line, (d) solve %
and gen_tok match div5 (identical rollout stream), (e) round_s ~8.8 s.
...
# Engine knobs
KV_POOL_GB   = _env_float("KV_POOL_GB", 28)  # V2: sized to measured peak, stays mapped
...
# V2: NO lend before train warmup — the small pool coexists with the train
# peak by construction; warmup doubles as the capacity gate (OOM here = pool
# too big for this card, shrink KV_POOL_GB).
...
try:
    for rnd in range(num_rounds):
        ...
        vmm_map_s = 0.0  # V2: pool never unmapped, nothing to reclaim
        ...
        vmm_unmap_s = 0.0  # V2: no lend — train runs with the pool mapped
